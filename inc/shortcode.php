...
function xevoq_demo_shortcode( $atts ) {
  ...
  foreach ( $media as $img ) {
    ...
    $html .= '<span class="icon-slide-'. esc_attr( $img->post_title ) .'-cursor"></span><span class="icon-slide-'. esc_attr( $img->post_title ) .' '. $alt_text .'"></span><span class="icon-slide-'. esc_attr( $img->post_title ) .'-text">'. esc_attr( $img->post_content ) .'</span>';
    $html .= '</li>';
  }
    $html .= '</ul>';
